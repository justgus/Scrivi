...
Result<bool> LocalFileSystem::exists(const AbsolutePath& path) {
    ...
    if (ec) return Result<bool>::failure({ErrorCode::ioError, ec.message(), path});
...
Result<void> LocalFileSystem::createDirectories(const AbsolutePath& path) {
    ...
    if (ec) return Result<void>::failure({ErrorCode::ioError, ec.message(), path});
    return Result<void>::success();
}

Result<Utf8Text> LocalFileSystem::readTextFile(const AbsolutePath& path) {
    std::ifstream in(path, std::ios::binary);
    if (!in) return Result<Utf8Text>::failure({ErrorCode::ioError, "could not open file", path});
    std::ostringstream ss;
    ss << in.rdbuf();
    if (!in && !in.eof())
        return Result<Utf8Text>::failure({ErrorCode::ioError, "read failed", path});
