...
| **I-0182** | `[Linux]` **A world object that can't be opened is named by its raw objectID, when its NAME is cached locally and available.** ⚠️ **Found by the USER on the REAL RIG during the SP-127 live pass, 2026-09-01** — the drive was pulled, a character double-clicked, and the panel reported the object as `character_01a0011f-af73-…` instead of *Myton*. ⚠️ **The name was NEVER unavailable.** ✅ **`binding.json` — which lives INSIDE the project, on local disk, NOT on the removed volume — carries `displayName: "Eskandar"` and a `cachedIndex` of all 35 objects with their `displayName`s** (`Myton`, `Veyra`, `Petch`, `Tintagael`, …). ⚠️ **So the app fell back to an ID while the answer sat in a file it had already read.** **Mechanism:** `SceneInspector.cpp:907-915` builds the pending-object message from `kRoleDisplayName`, and `worldDisplayName()` (`:686-695`) falls back to the raw `worldID` when `worldNames_` misses. ⚠️ **`worldNames_` is populated ONLY in the `refresh` path (`:393-402`) from `listWorlds`** — so any entry not present at that moment degrades to an ID. ⚠️ **The `:690` comment — *"Falling back to the ID is deliberate: an unnamed world is still better than an unattributed warning"* — is sound for a world that was NEVER named, but wrong here**, where a name is cached and simply not consulted. ✅ **Fix direction: resolve display names from the binding's `cachedIndex` (already local, already read) before falling back to any ID** — and ⚠️ **an ID should be the LAST resort, not the first fallback.** ⚠️ **A raw UUID is unreadable to a writer**, which is the whole point of the cache existing. ⚠️ **Related to [I-0181]** — both concern honest reporting when a world is away, but ⚠️ **this one is NOT latent: it was seen by the user on real hardware.** | Low | ✅ **SP-128** (⚠️ **assigned 2026-09-10** — ⚠️ **for its RIG VERIFICATION, T-0501; the fix itself is already in**) — found during SP-127 live pass | ✅ **VERIFIED 2026-09-11 (user-observed on the rig)** — ⚠️ **the only way this COULD be verified: the branch needs `listWorlds` to FAIL, which Docker cannot stage.** ✅ **With the share down, the status line names the world by its PACKAGE FOLDER, not a raw `world_…` UUID.** ⚠️ **Was RESOLVED - Not Verified since 2026-09-10** — ⚠️ **THIS RECORD'S ORIGINAL DIAGNOSIS WAS PARTLY WRONG AND IS CORRECTED HERE.** ⚠️ **It described the defect as an OBJECT named by its objectID** (`character_01a0011f-af73-…`) ⚠️ **and pointed at the pending-object message.** ✅ **The user corrected this 2026-09-10: the Scene Inspector's OBJECT names are CORRECT and do not change** (the core fills them from the binding's `cachedIndex` — `EndpointResolver.cpp:99`, contract at `RelationshipStore.hpp:58`: *the binding's cached name, never a bare ID*), ⚠️ **and the double-click dialog is a PLACEHOLDER.** ✅ **THE REAL SURFACE IS THE STATUS LINE**, `SceneInspector.cpp:736` — ⚠️ **which names a WORLD, not an object:** `World "world_01a0011f-af73-…" is offline. These 3 links are held pending.` ✅ **ROOT CAUSE: `worldNames_` is populated ONLY when `listWorlds` SUCCEEDS** (`:466`) — ⚠️ **so when the volume is down the map is EMPTY and `worldDisplayName()` fell through to the raw `worldID`** — ⚠️ **a UUID in the very sentence whose job is to explain the outage.** ✅ **FIX (user's own direction): fall back to THE PATH WE TRIED TO OPEN.** ⚠️ **`lastKnownPackagePath` is emitted REGARDLESS of status** (T-0419/[I-0137]) ✅ **and is now cached in `worldPaths_` on every healthy read**, ⚠️ **deliberately NOT cleared on a failed reload — that is exactly when it is the only readable identity we have** — ✅ **and cleared only on a PROJECT change, where another project's paths would be wrong.** ✅ **The status line now shows the package's FOLDER NAME** (`Eskandar`, suffix stripped) — ⚠️ **not the whole path, which would swamp a one-line panel** — ✅ **and the raw ID is now genuinely the LAST resort, which is what the original `:690` comment was right about.** ✅ **Docker build clean (271 targets); 22/22 Linux smokes green.** ⚠️ **NOT rig-verified: needs a live pass with the share down.** |
...
## Currently: **eleven records** — I-0147 (accepted limitation) + ⚠️ **I-0176, I-0177, I-0178, I-0180, I-0192, ⚠️ **I-0196 (HIGH — a 1,153-scene project freezes 321 s)**, ⚠️ **I-0197 (Apple bypasses ScriviCore)** — OPEN** + ✅ **I-0198 (VERIFIED — my T-0499 regression)** + ✅ **I-0199 (VERIFIED — my T-0500 flaw)** + ✅ **I-0182 (VERIFIED)** + 🟢 **I-0181, I-0191, I-0194, I-0195 (Resolved - Not Verified)**
