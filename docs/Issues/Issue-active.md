...
| **I-0203** | `[Apple]` ⚠️ **THE SCENE NAVIGATOR HIDES ITSELF when the window is not wide enough, and the writer cannot see it.** ⚠️ **Reported by the USER 2026-09-12:** *"the Scene Navigator hid itself and I couldn't see it."* ⚠️ **Almost certainly THE SAME DEFECT the user reported earlier as "the window, when not maximized, does not render the Navigation Bar"** — ✅ **filed separately from [I-0200] because that one is a COST defect and this is a LAYOUT defect**, ⚠️ **and folding them together is what I was warned against.** ---- ✅ **LIKELY CAUSE, read in the code, NOT yet proven:** ⚠️ **`EditorView.swift:201` uses `NavigationSplitView { … } detail: { … }` with `.navigationSplitViewStyle(.balanced)` and `.frame(minWidth: 700)`** — ⚠️ **but NO explicit sidebar width constraint** (`navigationSplitViewColumnWidth`), ⚠️ **and the macOS branch never sets `columnVisibility`** (a `@State` for it exists at `:46` but is wired only to the iOS branch at `:161`). ✅ **So AppKit is free to collapse the sidebar when the window narrows, and nothing forces it back.** ---- ⚠️ **NOT INTRODUCED BY [SP-132]** — ✅ **`SceneNavigatorView.swift` is UNMODIFIED from HEAD** (verified by `git status`), ⚠️ **and the earlier "Navigation Bar" report predates every change in this sprint.** ---- ⚠️ **WHY IT MATTERS MORE THAN IT SOUNDS: the navigator is the ONLY way to move between 1,153 scenes.** ⚠️ **A writer who narrows the window loses navigation entirely, with no affordance to bring it back and no indication it exists.** | **Medium** | ⚠️ **Unassigned** | 🔵 **Open — ⚠️ CAUSE READ IN CODE, NOT MEASURED.** ⚠️ **Needs a real resize to confirm** (`feedback_live_pass_finds_what_suites_cannot`). |
## Currently: **fourteen records** — I-0147 (accepted limitation) + ⚠️ **I-0176, I-0177, I-0178, I-0180, I-0192, ⚠️ **I-0197 (Apple bypasses ScriviCore)**, ⚠️ **I-0200 (HIGH — the app is UNUSABLE after the load)**, ⚠️ **I-0201 (HIGH — a launch ARGUMENT silently prevents startup)**, ⚠️ **I-0202 (a latent `assumeIsolated` trap)**, ⚠️ **I-0203 (the Scene Navigator hides itself)** — OPEN** + ✅ **I-0198 (VERIFIED — my T-0499 regression)** + ✅ **I-0199 (VERIFIED — my T-0500 flaw)** + ✅ **I-0182 (VERIFIED)** + 🟢 **I-0181, I-0191, I-0194, I-0195 (Resolved - Not Verified)**
