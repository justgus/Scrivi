...
| **I-0192** | `[Linux]` ⚠️ **A world whose drive has been PULLED still reports AVAILABLE — and a double-click on its object still returns SUCCESS — until a SCENE CHANGE forces a re-resolve.** ⚠️ **Found by the USER on the REAL RIG during T-0477's S3 physical yank, 2026-09-07.** ⚠️ **This INVERTS the failure mode EP-038 was hunting.** ✅ **Every layer beneath the app told the truth ON TIME**: the kernel removed the mountpoint `/run/media/justgus/SCRIVI-OTHE` immediately, and the core resolved `unavailable` correctly WHEN ASKED. ⚠️ **The app did not ask.** ⚠️ **So this is NOT a mount-layer lie (§2b) and NOT an SMB cache artefact (§2c) — it is a CACHED STATUS WITH NO INVALIDATION.** ⚠️ **The sharp edge is the double-click**: a stale label is bad, but ⚠️ **an affirmative SUCCESS message for an object on a drive that is physically gone is an active false claim to a writer.** ⚠️ **A writer who does not change scenes is told their world is fine while it is absent.** ⚠️ **T-0498 does NOT fix this** — it corrects what the core CONCLUDES, not when the app ASKS; ⚠️ **any future check that trusts cached status inherits the same defect.** ⚠️ **NOT YET MEASURED, and must not be guessed:** (1) ⚠️ **what the refresh trigger actually is** — *"change scenes"* is what the user did; whether it is scene selection, any navigator action, or an unrelated periodic refresh is UNKNOWN; (2) ⚠️ **whether the double-click SUCCESS came from cached status or from a read that spuriously succeeded** — ⚠️ **different defects, different fixes** (a page-cache hit is I-0174's territory; a cached STATUS is not); (3) ⚠️ **whether an FD was stranded** — ⚠️ **the 2b container pass found a held FD surviving `umount -l` ENTIRELY**, so ⚠️ ***"the editor still works" is NOT evidence the volume is present.*** ⚠️ **No suite could have caught this** — it is about WHEN a question is asked, not what the answer is (`feedback_live_pass_finds_what_suites_cannot`). | **Medium** | ⚠️ **Unassigned** — found during T-0477 S3 | 🔵 **Open** |
...
## Currently: **ten records** — I-0147 (accepted limitation) + ⚠️ **I-0176, I-0177, I-0178, I-0180, I-0181, I-0182, I-0192 — all OPEN** + 🟢 **I-0191 (Resolved - Not Verified)**

⚠️ **I-0192 came out of T-0477's S3 physical yank (2026-09-07) — and the INSTRUMENTATION DID NOT FIND IT.** ⚠️ **The probe was run on the WRONG MACHINE because the runbook never said which machine each command belonged to**, so it watched a Linux path on the MacBook and produced noise. ✅ **The user found the defect by PULLING THE DRIVE AND WATCHING THE APP.** ⚠️ **No probe in T-0477 §4's table would have caught it anyway** — ✅ **every one of them questions the OS, and the OS was honest the whole time.** ⚠️ **Compare `feedback_live_pass_finds_what_suites_cannot`: the same lesson, one layer further out — ⚠️ **a green suite never means usable, and NEITHER DOES CORRECT INSTRUMENTATION.**
