...
| **I-0171** | `[Build]` ⚠️ **Both `.dockerignore` files exclude only `build/`, while FIVE build directories exist in the working tree** — so a container build is still poisoned by host `CMakeCache.txt` files, exactly as SP-121 described. ⚠️ **REPRODUCED LIVE 2026-08-25** during SP-122 T-0468: a `docker build --no-cache` failed with *"The current CMakeCache.txt directory /src/build-tests/CMakeCache.txt is different than the directory /Users/justgus/... where CMakeCache.txt was created"*. ⚠️ **The leaking directory was `build-tests/`, dated Aug 21 — PRE-EXISTING, not created by this sprint.** Present in the tree: `build/`, `build-tests/`, `build-linux-tests/`, `build-iphoneos/`, `build-iphonesimulator/`. ⚠️ **SP-121's fix was incomplete because it matched the ONE directory it had seen** rather than the pattern. ✅ **`platforms/linux/docker/Dockerfile.dockerignore` ALREADY EXISTED and is the stronger file** (it also carries `**/build/` and `**/.build/`, the ~0.5 GB Apple tree that once exhausted the Docker copy) — ⚠️ **but Docker applies `<dockerfile>.dockerignore` ONLY for that Dockerfile**, so any other Dockerfile falls back to the weaker root file. ⚠️ **Fix must cover `build*/`, not enumerate names** — and the two files' relationship must be ruled, not left as a silent duplicate. | Medium | SP-122 | 🔴 **Open** — ⚠️ **found by RUNNING the leg, not by reading the file** |

## Currently: **two records** — I-0147 (accepted limitation) + ⚠️ **I-0171, OPEN**
