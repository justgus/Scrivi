# Closed Epic — EP-040 `[Apple]` The Editor Shell

## EP-040: `[Apple]` ⚠️ **The Editor Shell** — ✅ **build Scrivi as the kind of app it actually is**

> ⛔ **EPIC CLOSE AUTHORIZED BY THE USER 2026-09-24 — ⚠️ AND BLOCKED ON TWO THINGS CLAUDE MAY NOT
> DECIDE. ✅ Both are named here rather than closed over.**
>
> ✅ **CLOSED 2026-09-24 (user-approved).** ⚠️ **ALL THIRTEEN ACs RESOLVED: twelve MET, ✅ AC10 RULED
> OBE.** ✅ **Five Sprints: [SP-134] · [SP-135] · [SP-136] · [SP-137] · [SP-150], all closed.**
> ✅ **An AUDIT CHECK was run before the close** (`../Audits/Audit-Check-20260924.md`) — ⚠️ **it found
> THREE inconsistencies, all ruled by the user and remediated in the same step.**
> ⛔ **[I-0206] REMAINS OPEN and is NOT closed by this Epic** — ✅ **AC11 accepted it as a limitation
> WITH a measurement, which is that clause's second branch, not a fix.**
...
- [x] **AC8** — ✅ **MET by [SP-137], CLOSED 2026-09-23.** ✅ **The Object Detail Sheet no longer hand-builds window chrome**, ⚠️ **per a RULED
...
- [x] **AC11** — ✅ **MET 2026-09-24** (⚠️ **[I-0213] FIXED; [I-0206] accepted as a limitation WITH a measurement — ⛔ the Issue itself stays OPEN**). ⚠️ **CARRIED FROM [EP-039] (user ruling 2026-09-15): the manuscript surface's
      ...
      was wrongly closed once on a sample taken ~1% in.** ✅ **[I-0213] IS SETTLED — VERIFIED BY THE
      USER 2026-09-24 and ACCEPTED AS A LIMITATION WITH A MEASUREMENT** (`createChapter WORK=396.7 ms`
      steady-state on 1,179 scenes, ✅ **measured DELIBERATELY ON A USB MOUNT so the figure is a
      WORST-CASE FLOOR — internal storage can only be faster**; ⚠️ **`reloadSceneDots` was NEVER
      instrumented internally, and that is stated rather than claimed done**).
      ⚠️ **AC11 therefore rests on [I-0206] ALONE** (⚠️ *the offset-linear `setSelectedRange`,
      `57–81 ms` near the end of a 1.85 MB document*).
      ---- ✅ **[I-0213]'s REMAINDER IS NOW MEASURED AT THE CORE 2026-09-24** (new test
      `StructuralOpIndexRebuildTests.cpp`): ⚠️ **a `createChapter` costs 50 filesystem calls, but the
      `ProjectIndex` rebuild it FORCES costs 866 on a 400-scene project — ~17x the op itself, and it
      SCALES with manuscript size while the op does not.** ⚠️ **Cause: every structural op drops the
      cached index** (⚠️ *deliberately — it fixed [EP-039] AC5b, where a write landed on disk and the
      next read reported `count:0`*) ⚠️ **so the next story-time read re-walks the WHOLE manuscript.**
      ✅ **FIXED 2026-09-24 by [SP-150]** ([T-0549] + [T-0550]) — ⚠️ **and USER-VERIFIED on the USB rig:
      `reloadSceneDots` ~235 ms → `1.4 ms`.**
      ---- ⚠️ **AC11 IS THEREFORE MET FOR [I-0213] AND *NOT* FOR [I-0206]**, ✅ **and that is stated
      rather than swept:** ⛔ **[I-0206] (every keystroke ~59 ms inside AppKit; `setSelectedRange`
      linear in document offset) IS UNTOUCHED BY THIS SPRINT.** ⚠️ **The user found typing acceptable
      at the document end on 2026-09-24 (`[SCRIVI-KEY] total=2.4–3.2 ms` at `len≈1,852,000`), ✅ which
      is AC11's "ACCEPTED AS A LIMITATION WITH A MEASUREMENT" branch — ⛔ but [I-0206] REMAINS OPEN as
      its own record and is NOT closed by this Epic.**
...
- [x] **AC10** — ⛔ **RULED OBE (overtaken by events) BY THE USER 2026-09-24. ✅ THE GUARD WAS NOT
      BUILT, AND THAT IS A DECISION, NOT AN OMISSION.**
      ⚠️ **ORIGINAL TEXT:** *"A CI guard prevents regression: a new bar added as a bare `VStack`
      sibling should be caught, the way [T-0527] guards the TextKit downgrade. ⛔ Mechanism NOT yet
      designed — the guard must be SPECIFIED before it is promised."*
      ⚠️ **THE USER'S REASONING, RECORDED VERBATIM BECAUSE IT SETS A PRECEDENT:** *"I'm not sure a
      grep guard is necessary here. Seems like a lot of effort to go through just to [ensure] the AI
      complies with the design, which it [ought] to do anyway."*
      ✅ **THE DISTINCTION THAT MAKES THIS DIFFERENT FROM [T-0527]:** ⚠️ **T-0527 guards a SILENT
      RUNTIME DOWNGRADE — TextKit 1 vs 2 is invisible in review and shows up only as a performance
      cliff.** ✅ **A bar added as a bare `VStack` sibling is VISIBLE IN THE DIFF**, ⚠️ **so the failure
      mode it guards is a review miss, not an undetectable regression.**
      ⛔ **WHAT THIS ACCEPTS: nothing mechanically prevents the [I-0203] structure returning** —
      ✅ **[SP-135]'s fix and the app-shape study are the record that says why it must not.**
      ⚠️ **Consistent with [SP-137]'s T-0548, where the user REMOVED the runtime half of a similar
      guard and kept only the static check.**

- [x] **AC12** — ✅ **MET AND USER-VERIFIED 2026-09-24** ([T-0549]). ✅ **ADDED 2026-09-24 with [SP-150] (user-directed).** ⚠️ **A structural op that changes
      ONLY one scene's DATA does NOT force a manuscript-wide index rebuild.** ✅ **MEASURED IN FILESYSTEM
      CALLS, NOT WALL CLOCK** (⚠️ *a warm page cache hides this class; the user's rig is a USB mount*):
      ⚠️ **a `set_scene_story_time` followed by a story-time read costs ZERO manuscript reads** —
      ⛔ **today it costs 866 on a 400-scene project.** ✅ **Served by [T-0549] (Class A).**
      ⛔ **[EP-039] AC5b MUST STILL HOLD** — ✅ **patch-or-drop: any partial path that fails drops the
      WHOLE index, and the next query is correct.**

- [x] **AC13** — ✅ **MET AND USER-VERIFIED 2026-09-24** ([T-0550]). ⚠️ **MEASURED ON THE USER'S USB RIG: `reloadSceneDots` ~235 ms → `1.4 ms` (~170x) on 1,178 scenes.** ✅ **ADDED 2026-09-24 with [SP-150] (user-directed).** ⚠️ **A structural op that changes
      manuscript ORDER or MEMBERSHIP re-reads ONLY the affected chapter, not the whole manuscript.**
      ✅ **Served by [T-0550] (Class B) — ⚠️ THIS is the half that moves the chapter-create cost the
      user measured.** ⛔ **THE EXACT BOUND IS SET FROM THE MEASUREMENT, NOT INVENTED IN ADVANCE.**
      ⛔ **THE RISKY HALF: partial rebuilds interact with ORDER KEYS, and [EP-027] made manuscript order
      FILESYSTEM-AUTHORITATIVE for reasons that bit before.** ⚠️ **BLOCKED ON [T-0549].**

### Sprints — ✅ **[SP-150] CLOSED 2026-09-24**
...
| ✅ **SP-137** | **S6** | ✅ **The Object Detail Sheet** — ✅ **hosting RULED (keep the modal sheet); real chrome + a layout guard.** ✅ **Q4 RULED 2026-09-23: static guard + debug assertion** | ✅ **CLOSED 2026-09-23** | ⚠️ **MEDIUM** |
| ✅ **SP-150** | — | ✅ **[I-0213]'s accepted remainder** — ⚠️ **stop a structural op re-walking the WHOLE manuscript** (⚠️ *`[ScriviCore]` work under this `[Apple]` Epic: the cost is paid by the editor shell's `reloadSceneDots`*) | ✅ **CLOSED 2026-09-24 (user-approved)** — ✅ **T-0549 + T-0550 VERIFIED; AC12/AC13 MET** | ⚠️ **MEDIUM** (⛔ **T-0550 the risky half**) |
...
| **I-0213** | ⚠️ **Chapter create costs `~400 ms` of work ON A USB MOUNT** (⚠️ *originally a 2.7 s freeze on 1,174 scenes*; `reloadSceneDots` ~two-thirds of what remains) | ✅ **VERIFIED BY THE USER 2026-09-24 ON A DELIBERATE WORST-CASE RIG** — ⚠️ **ACCEPTED AS A LIMITATION WITH A MEASUREMENT** (AC11's second branch). ✅ **ARCHIVED** → [`../Issues/Verified/Issue-verified-0211-0220.md`](../Issues/Verified/Issue-verified-0211-0220.md) |
...
### Completion Summary — ✅ **CLOSED 2026-09-24 (user-approved)**

✅ **WHAT THE EPIC SET OUT TO DO:** ⚠️ *"A writer can see and reach every part of the editor"* —
✅ **build Scrivi as the kind of app it actually is, rather than hand-building what the platform
already provides.**

✅ **WHAT SHIPPED, SPRINT BY SPRINT:**
- ✅ **[SP-134]** — the toolbar: a real `NSToolbar` with title/subtitle and the existing verbs surfaced.
- ✅ **[SP-135]** — the bars: `safeAreaBar` conversion. ⚠️ **This closed [I-0203], whose root cause was
  that the banner, Timeline and Inspector tab bar were all `VStack` SIBLINGS of the content** —
  ✅ **ONE structural mistake that read as FOUR separate bugs.** ✅ **[I-0205] answered from code.**
- ✅ **[SP-136]** — the Inspector as a real `.inspector` column, user-resizable width preserved.
- ✅ **[SP-137]** — the Object Detail Sheet stopped hand-building window chrome.
  ⚠️ **T-0547 took FOUR implementations and produced THREE Issues** ([I-0248] · [I-0249] · [I-0250]).
- ✅ **[SP-150]** — ⚠️ **[I-0213]'s accepted remainder.** ✅ **`reloadSceneDots` ~235 ms → `1.4 ms`
  (~170x) on 1,178 scenes, measured on the user's USB rig.**

⚠️ **WHAT WAS RULED RATHER THAN BUILT:**
- ⛔ **AC10 (a CI guard against bars re-added as bare `VStack` siblings) was RULED OBE by the user.**
  ✅ **The distinction from [T-0527]: that guard catches a SILENT runtime downgrade; ⚠️ a misplaced bar
  is VISIBLE IN THE DIFF.** ⛔ **This accepts that nothing mechanically prevents [I-0203]'s structure
  returning.**
- ⚠️ **AC7's Export and text-size lens are DECLARED SLOTS ONLY, documented unimplemented.**
- ⛔ **SP-138 (`NSSplitViewController` rebuild, step S5) was NEVER SCHEDULED** — ✅ **the Epic's goal was
  met without it.**

⛔ **WHAT THIS EPIC DOES NOT CLOSE:**
- ⚠️ **[I-0206]** (*every keystroke ~59 ms inside AppKit; `setSelectedRange` linear in document
  offset*) **REMAINS OPEN.** ✅ **AC11 took its "accepted as a limitation WITH a measurement" branch,
  on the user's 2026-09-24 pass showing `[SCRIVI-KEY] total=2.4–3.2 ms` typing at the document end.**
- ⚠️ **[I-0244]** (*the Linux app has none of this Epic's editor shell*) **is FILED and OPEN** —
  ✅ **`feedback_linux_adopts_apple_shape`: Apple is the reference, and Linux owes the same shape.**
- ⛔ **Six Class B C-ABI endpoints still drop the whole `ProjectIndex`** because their result structs
  do not name the affected chapter — ✅ **a ruling recorded at the call site, and separate work.**

⚠️ **THE METHOD LESSON THIS EPIC PRODUCED, WORTH CARRYING FORWARD:** ⛔ **[SP-150]'s T-0550 shipped an
implementation that built clean, passed every test, and DID NOTHING** — ⚠️ **the guard's before-drop
destroyed the very index the partial rebuild needed, so the fast path never ran, while the perf test
still reported 13.7x because it measured the index layer directly rather than through the ABI.**
✅ **A PROBE found it and a NEGATIVE CONTROL proved the fix.** ⚠️ **`feedback_prove_code_is_reached`,
fourth occurrence — ✅ and the first time the check ran BEFORE success was reported.**
