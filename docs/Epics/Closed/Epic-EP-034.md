...
**Status:** ✅ **CLOSED 2026-08-25** (user-approved) — active 2026-08-20 → 2026-08-25; ⚠️ **widened in place** by the design doc
**Sprints:** **SP-115 – SP-122** (eight, all closed) · **ACs:** ⚠️ **eleven met, AC11 MOVED to EP-035**
...
(SP-115 ✅ · SP-116 ✅ · SP-117 ✅ · SP-118 ✅ · SP-119 ✅ · SP-120 ✅ · **SP-121 ✅**); ✅ **SP-122 CLOSED — the LAST.** ⚠️ **All twelve ACs resolved; EP-034 is 🟠 COMPLETE awaiting close approval**
**Actual Close Date:** —
...
- [→] ⚠️ **AC11 — MOVED OUT OF THIS EPIC to [EP-035] by user ruling 2026-08-24.** ⚠️ **It was never a
      parity criterion.** Verified at SP-121 planning: the Linux app has **ZERO** call sites for any
      object, edge, world or asset endpoint, and `SceneInspector.cpp` is a **67-line stub** — against
      **~4,800 lines** of Apple surface across 11 files. ⚠️ **AC1 presumes a Kind Card list item to
      double-click and Linux has no Kind Cards**, so AC1–AC9 rest on a layer that was never built there.
      ✅ **SP-121 instead completes `ScriviBridge`** (the Linux `ScriviEngine`) so EP-035 is a UI port and
      not bridge archaeology. ⚠️ **EP-034 does NOT claim Linux parity** and must not be read as having
      delivered it.
- [x] ✅ **AC12 — MET (SP-122, 2026-08-25).** All five legs **RUN, not cited**: arm64 **567/567**;
      ⚠️ **x86-64 567/567** (binary confirmed `Mach-O x86_64`); ⚠️ **sanitizers 567/567** (confirmed
      linked — 55 `__asan`/`__ubsan` symbols vs **0** in the plain build); interop **127 tests / 12
      suites** + app **BUILD SUCCEEDED**; ⚠️ **Linux `ctest` ACTUALLY RAN — 571/571, NON-ROOT
      (`uid=1001`)**, in a second image built `SCRIVI_BUILD_TESTS=ON`.
      ⚠️ **The first run used Xcode 26.6 and was DISCARDED** — the project requires Xcode 27
      (AppleClang 21), a different compiler, so that result was void rather than adjustable.
...
| **SP-121** | ⚠️ **`[Linux]`** Complete `ScriviBridge` to ABI parity (**34 → 81 of 100**) + **the Porting Outline** — ⚠️ **NOT AC11, which moved to EP-035** | ✅ **Closed 2026-08-25** — [record](../../Sprints/Closed/Sprint-SP-121.md) | ⚠️ **No AC** — AC11 left this Epic; verified by suite, ⚠️ **not by use** |
| **SP-122** | ⚠️ **`[Cross]`** AC verification + ⚠️ **the LIVE-USE pass** + Epic close prep | ✅ **Closed 2026-08-25** — [record](../../Sprints/Closed/Sprint-SP-122.md) | ✅ **AC12** — five legs green + a clean live pass |
...
| **T-0460** | ⚠️ **Gap audit** — all **100** endpoints, each with a disposition | SP-121 | **High** | ✅ **Verified** |
| **T-0461** | Objects + kinds + worlds (8+7+1) | SP-121 | **High** | ✅ **Verified** |
| **T-0462** | Edges + relation types + assets (4+1+3) | SP-121 | **High** | ✅ **Verified** |
| **T-0463** | Scene metadata, comments, inbox, repair, snapshots | SP-121 | Medium | ✅ **Verified** |
| **T-0464** | ⚠️ **Bridge tests — through `ScriviBridge`, not the C ABI** | SP-121 | **High** | ✅ **Verified** |
| **T-0465** | ⚠️ **THE PORTING OUTLINE** — written for **five** platforms | SP-121 | **High** | ✅ **Verified** |
| **T-0466** | **L1 + L4** — `ctest` macOS arm64, interop, app build | SP-122 | **High** | ✅ **Verified** |
| **T-0467** | ⚠️ **L2 + L3** — x86-64 + sanitizers — ⚠️ **the two legs that hide defects** | SP-122 | **High** | ✅ **Verified** |
| **T-0468** | **L5** — Linux container `ctest`, ⚠️ **tests ON, NON-ROOT, re-run** | SP-122 | **High** | ✅ **Verified** |
| **T-0469** | ⚠️ **THE LIVE-USE PASS** on the real rig — ⚠️ **the primary instrument** | SP-122 | **High** | ✅ **Verified** |
| **T-0470** | ⚠️ **Re-verify AC1–AC10 against their own triggers**, not re-cite | SP-122 | **High** | ✅ **Verified** |
| **T-0471** | **Epic close prep** — summary + ⚠️ **Audit Check** (read-only) | SP-122 | Medium | ✅ **Verified** |
...
## ✅ EP-034 — COMPLETION SUMMARY (✅ **CLOSED 2026-08-25, user-approved**)

✅ **CLOSED 2026-08-25 on user approval**, together with rulings on both unowned items (see below).

### Status: **all twelve ACs resolved** — eleven met, one moved out

✅ **AC1–AC10 met** and **re-verified 2026-08-25** against their own triggers, not re-cited.
✅ **AC12 met** — five suite legs, each run.
⚠️ **AC11 was MOVED to [EP-035] by user ruling 2026-08-24** — ⚠️ **it was never a parity criterion**: the
Linux app had **zero** object/edge/world/asset call sites and a **67-line stub** inspector against
~4,800 lines of Apple surface. ⚠️ **EP-034 does NOT claim Linux parity and must not be read as having
delivered it.**
...
| Sprint | Delivered | AC |
| ------ | --------- | -- |
| **SP-115** | Five carried Issues + I-0142 | AC10 |
| **SP-116** | World assets, `assetPath`, ⚠️ **the kind-scope endpoint (D5)** | — |
| **SP-117** | ⚠️ **Detail Sheet shell** — the Epic's first writer-facing surface | AC1, AC9 (half) |
| **SP-118** | Related objects + relationship creation | AC5, AC6, AC7 |
| **SP-119** | Images + `tags` | AC2, AC3, AC4, AC9 |
| **SP-120** | Source creation + footnote text | AC8 |
| **SP-121** | ⚠️ **`ScriviBridge` 34 → 81 of 100** + **the Porting Outline** | ⚠️ **none — AC11 left** |
| **SP-122** | AC verification + the live pass + close prep | AC12 |

### ⚠️ What this Epic proved about its own process

⚠️ **22 consecutive Issues across SP-118–SP-120 came from a writer clicking. NONE came from any suite.**
✅ **Every one of those sprints shipped a NEW surface**, which is the condition under which that holds.

⚠️ **SP-122 initially mis-generalised that statistic** — it was written expecting the regression pass to
find defects. ✅ **User ruling corrected it:** *"We vetted all the features in the Apple App and you are
having me do a regression test verbatim from previous tests. I would expect no failures."*
⚠️ **A regression pass over vetted features is not a discovery pass, and clean is the correct outcome.**

### ⚠️ What is NOT closed by this Epic

| Item | Owner |
| ---- | ----- |
| ⚠️ **AC11 — the entire Linux object layer** | **EP-035** (🔵 Proposed, ⚠️ **not scheduled**) |
| ⚠️ **SP-121's 47 bridged endpoints have NO live click-through** | **EP-035** |
| ⚠️ **`upsert_relation_type` — bridged, unreached on BOTH platforms** | **EP-035** |
| ⚠️ **Three timeline endpoints unreached on Apple** (`resolve_timeline_project_times`, `set_timeline_epoch_offset`, `set_world_epoch_offset`) | ⚠️ **Unowned** |
| ⚠️ **I-0171** — both `.dockerignore` files match one pattern against five build dirs | ⚠️ **Open, unassigned** |
| **history (15) + buffers (4) endpoints** | **EP-019** |
| **I-0147** — 60 s world-unwritable window | ⚠️ Deferred — network-worlds design |

⚠️ **I-0172 is Verified but was proven by COMPILATION, not by exercising the fork popover.**

### ⚠️ Before closing, note the honest limits

1. ⚠️ **SP-121's bridge work is suite-proven only.** 47 endpoints, no human has used one.
2. ⚠️ **The three unreached Apple timeline endpoints have no owner** — they should get one at EP-035
   planning or be explicitly ruled dead.
3. ✅ **The Audit Check found the Issue index FOUR SPRINTS STALE** — I-0140/I-0141 shown open in a sprint
   that closed 2026-08-21. ⚠️ **Without it this Epic would have closed against an index claiming open
   Issues that were resolved four sprints earlier.**
...
*Last Updated: 2026-08-24, fourth pass (**SP-121 🟡 ACTIVATED**, and ⚠️ **AC11 MOVED OUT of this Epic to
EP-035 by user ruling.** ⚠️ **AC11 was never a parity criterion**: Linux has **zero** object/edge/world/
asset call sites and a **67-line stub** inspector, against ~4,800 lines of Apple surface — an entire Epic,
not a sprint. ✅ **SP-121 completes `ScriviBridge` instead** — **100** endpoints exist (⚠️ **not 102**), Apple reaches **96**,
Linux 35; **43 in scope, 19 history/buffers excluded to EP-019.** ⚠️ **EP-034 now closes on AC12 alone and
must NOT be read as having delivered Linux parity.** ⚠️ **T-0465 delivers a PORTING OUTLINE written for
five platforms** — iPad, iPhone, Windows and visionOS all re-run this. Prior note follows.)*
...
## ✅ Closing rulings — 2026-08-25

⚠️ **EP-034 was NOT closed with loose ends unnamed.** Both unowned items found at SP-122 were ruled by the
user in the same session, and both are now tracked Tasks.

### Ruling 1 — `scrivi_upsert_relation_type` is a **REAL FEATURE owed a surface**

⚠️ **Fully implemented in the core, bridged on Linux by SP-121, and NEVER called on any platform.**
Projects are seeded with default relation types and `list_relation_types` reads them — ⚠️ **nothing can
ADD one.**

> ✅ **RULED: a real feature owed a UI — file it.** Custom relationship vocabulary is a legitimate
> worldbuilding capability (`sworn-enemy-of`, `liege-of`), not speculative code.

→ **T-0472**, assigned to **EP-035**, 🔵 Backlog. ⚠️ **A C ABI test is owed with it** — the endpoint has
none, and `feedback_boundary_tests_not_facade` is why that matters.

### Ruling 2 — the three Apple timeline endpoints: **parity IS owed, but unscheduled**

`resolve_timeline_project_times`, `set_timeline_epoch_offset`, `set_world_epoch_offset` — unreached on
Apple, bridged on Linux. ⚠️ **The two epoch setters have core implementations AND core tests**; Linux's
Timeline Panel (1,422 lines) is the mature surface and needed them first.

> ✅ **RULED: file an Apple timeline-parity Task** so the gap is tracked rather than rediscovered at
> every audit.

→ **T-0473**, ⚠️ **carrying NO Epic by ruling** — a tracked debt, ⚠️ **not a commitment in any Epic.**

---

## ⚠️ What this Epic did NOT deliver — stated plainly at close

1. ⚠️ **NO Linux parity.** AC11 left for **EP-035**. ⚠️ **EP-034 must never be cited as having delivered
   a Linux object layer** — at close, Linux still has zero object surfaces and a 67-line stub inspector.
2. ⚠️ **SP-121's 47 bridged endpoints are SUITE-PROVEN ONLY.** No human has used one. ⚠️ **If EP-035
   never runs, that work is 47 unreached functions** — named here so it cannot be quietly forgotten.
3. ⚠️ **I-0171 is OPEN and unassigned** — both `.dockerignore` files match one pattern against five build
   directories, so a container build is still poisonable.
4. ⚠️ **I-0172 was Verified by COMPILATION, not by exercising the fork popover.**

---

*Closed 2026-08-25 on user approval. ⚠️ **Eight sprints (SP-115–SP-122), eleven ACs met, one moved out.***
