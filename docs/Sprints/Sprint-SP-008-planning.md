...
**Goal:** Implement the serialization foundation and facade entry point for the repair lifecycle. Deliver `RepairIssueJson` schema module (T-0028) and `ScriviCore::applyRepair()` with full request/result types and a dispatch table (T-0029). After this sprint, C++ code can serialize `RepairIssue` lists to JSON and call `applyRepair()` with stub handlers.
...
| ------ | ----- | -------- | ------ |
| T-0028 | `RepairIssueJson` Schema Module | High | 🟠 Implemented - Not Verified |
| T-0029 | `applyRepair` Facade Method — Request, Result, and Dispatch | High | 🟠 Implemented - Not Verified |
...
- Sprint ends when `cmake --build build --parallel` compiles cleanly and `ctest` passes with the new schema round-trip tests.
- SP-009 depends on this sprint being complete.
