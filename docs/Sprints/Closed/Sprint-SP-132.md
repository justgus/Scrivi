...
| **T-0521** | ⛔ **NOT STARTED — ⚠️ the status cell below says `Implemented - Not Verified` and that is WRONG (verified against the code 2026-09-15).** ✅ **Tracked in [`Task-unverified.md`](../../Tasks/Task-unverified.md).** ---- ✅ **`[I-0200]` Remove `rebuildStorage`'s O(N²) chapter lookup** (`:612-617`) — ⚠️ **precompute chapterID → title and chapter ordinals ONCE per rebuild.** ✅ **[I-0196]'s third defect** | **Medium** | 🔵 Not started | 🟢 **Implemented - Not Verified** |
