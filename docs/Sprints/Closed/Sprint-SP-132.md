...
✅ **DISPOSITION RULED 2026-09-15 (user).** ✅ **[T-0520] LANDED** (as two `Hasher` digests, ⚠️ **not** the specified revision counter — a variation, not a gap). ✅ **[T-0521] VERIFIED** (see its row). ✅ **[T-0519] CLOSED AS SUPERSEDED** — ⚠️ **its two mechanisms were reverted because the build launched with NO WINDOW, ✅ and its TARGET was then achieved by OTHER MEANS in [EP-039]** (navigator click `35–45 s` → `~0.3 s`). ⚠️ **This sprint's own text already said the per-row hypothesis was UNPROVEN and `NSTableView` reentrancy was the stronger suspect** — ✅ **so there is no negative left to prove, and NO mitigation Sprint is warranted.**
...
| **T-0521** | ✅ **VERIFIED 2026-09-15 (user-ruled).** ✅ **`[I-0200]` Remove `rebuildStorage`'s O(N²) chapter lookup.** ⚠️ **A PRIOR EDIT ON THIS LINE CLAIMED "NOT STARTED" — THAT CLAIM WAS WRONG AND IS RETRACTED.** ✅ **The literal `first(where:)` and `allScenes.map(\.chapterID)` ARE still present at `ManuscriptTextView.swift:624-635`, ⚠️ but the `if !t.isEmpty { return t }` guard on the line ABOVE returns first for every titled chapter, so the O(N) map rebuild NEVER RUNS.** ✅ **The pathological cost is gone; ~55 boundary scans, not ~130,000.** ⚠️ **The earlier claim read code SHAPE, not CONTROL FLOW.** |
...
⚠️ **THE HONEST POSITION AS WRITTEN 2026-09-14: T-0519's ACTUAL TARGET — stopping one click from invalidating ~1,200 rows — IS NOT DONE.** ⚠️ **Both mechanisms reached for were wrong.**

✅ **SUPERSEDED 2026-09-15.** ✅ **[EP-039] eliminated the SYMPTOM by other means** — ⚠️ **the index work, not per-row scoping.** ✅ **Navigator click went `35–45 s` → `~0.3 s`.** ⚠️ **Since nothing ever demonstrated the per-row read caused the stall, and the stall is now GONE, no measurement could show this work is still needed.** ⛔ **T-0519 is CLOSED, not carried.**
