# SP-089: [Apple] EP-029 — editor wiring ✅ CLOSED

**Status:** ✅ **Closed 2026-08-03 (Human-approved).** The final EP-029 sprint — closes the Epic. All four
tasks Verified: T-0354 (Pass A, 2026-07-29), T-0357 (title-capture + chapter promotion, 2026-07-29), T-0355
(Pass B structured buffers, 2026-08-03), T-0356 (AC6 reversible structured undo, 2026-08-03). AC1–AC7 all met.
ScriviCore `ctest` **369/369**, macOS interop **45/45**, app **BUILD SUCCEEDED**. Archived alongside
`../../Epics/Closed/Epic-EP-029.md`.

---

## SP-089: [Apple] EP-029 — editor wiring (closed)

**Status:** ✅ **Closed 2026-08-03 (Human-approved)** — activated 2026-07-27. The final EP-029 sprint — wires the manuscript editor's
...
| T-0356 | **Reversible structured undo (AC6)** — extend C++ `HistoryService` for `structuredCut`/`structuredPaste` inverse-op undo/redo + tests (must also restore promoted scenes' chapter membership + survivor chapter title — T-0357) | ✅ **Verified (2026-08-03)** — user confirmed the basic undo features pass live verify. App-runs-inverse: `HistoryService` stays in-memory; a `structural` node kind carries an opaque inverse payload, undo/redo step ACROSS it returning `structuralInverse:{direction,payload}`; the app replays the inverse fragment op + reload. New core endpoint `scrivi_fragment_uncut_paste`. ctest **369/369** (7 new T-0356), interop **45/45**, macOS **BUILD SUCCEEDED**. |
...
### T-0356 implemented (2026-08-03) — AC6 reversible structured undo

**Architecture (user-ruled 2026-08-03): app-runs-inverse.** `HistoryService` stays a pure, in-memory,
filesystem-free index (its header invariant preserved). A cross-boundary cut/paste now records a **reversible
`structural` node** (new `EventKind::Structural`) carrying an **opaque inverse-op payload** the app minted.
`undo()`/`redo()` **step ACROSS** it (unlike a hard barrier) and return `structuralInverse:{direction,payload}`;
the app replays the inverse fragment op on disk + `reloadManuscriptFromDisk` (the verified T-0354 machinery). The
engine never parses the payload. (Rejected: engine-runs-inverse-on-disk — would break the in-memory invariant.)

**The four inverse directions reduce to two disk ops** (§5 inverse-op framing):
- **undo-cut** / **redo-paste** → a **paste** (`fragmentPaste` of the fragment at the caret).
- **undo-paste** / **redo-cut** → an **uncut** via the new core endpoint `scrivi_fragment_uncut_paste` — folds the
  created scenes back into the target and **strips the pasted piece texts by length** (plain concatenation would
  leave the pasted text behind — the sharp edge), then deletes the created scenes/chapters. Keyed by IDs + the
  fragment, so the app does **no byte-span math** (user-ruled: full symmetry, engine computes spans).
- redo-of-a-cut needs the IDs the undo-cut re-pasted → cached on the coordinator, keyed by the payload's stable
  identity (session-local; a redo only follows an undo in one run).

**Landed:** `EventKind::Structural` + `EventNode.structuralPayload` + `carriesDiff()` guard at all
evict/prune/head-rebuild sites (`HistoryService`); `recordBarrier` routes to a structural node when a payload is
present; `HistoryStore` persists/rehydrates the kind + payload; C ABI `record_barrier` reads an optional
`structuralPayload`, `undo`/`redo` emit `structuralInverse`; new `scrivi_fragment_uncut_paste` (+
`FragmentPaster::uncutPaste`); Swift `HistoryStructuralPayload`/`HistoryStructuralInverse` + `fragmentUncutPaste`
wrapper; the three record sites (⌘X, ⌥N buffer-cut, ⌘V/⌃N paste) build the payload; `apply(step:)` runs the
inverse. **Tests:** 3 `HistoryService` unit + 1 C ABI + 3 `uncutPaste` integration (paste→uncut restores
byte-for-byte) + 1 Swift interop. ctest **369/369**, interop **45/45**, macOS **BUILD SUCCEEDED**.

**✅ Verified (2026-08-03):** user confirmed the basic undo features pass live verify — cross-boundary ⌘X then
⌘Z restores text and structure; ⌘V then ⌘Z removes the created scenes/chapters. (Deeper edge cases — multi-cycle
undo/redo, redo-of-a-cut, reopen-after-undo — not separately exercised in this pass; raise an Issue if any surface.)
