...
| **T-0501** | ⚠️ **LIVE PASS on the real rig** — ⚠️ **[I-0182] READ with the share DOWN**, ⚠️ **[I-0195] progress WATCHED on `cache=none`.** ✅ **Plus `ctest` + smokes non-root ON THE RIG** | **SP-128** | 🟠 **UNBLOCKED but NOT READY (rechecked 2026-09-10).** ✅ **The rig is POWERED ON and reachable** — ⚠️ **`ssh` is SLOW to answer (>6 s), which earlier read as "timed out"; a 20 s timeout connects fine.** ⚠️ **THE REAL BLOCKER IS NOW A STALE CHECKOUT:** the rig sits at `15f26c7` and is **7 commits behind**, ⚠️ **missing [I-0182]'s fix — one of the two things this Task must verify.** ✅ **Ready when: bump the stamp on the Mac → commit → push → `git pull` on the rig → rebuild → confirm `scrivi_linux --version`** (`feedback_rig_is_read_only`, `feedback_confirm_the_build_under_test`). ⚠️ **Nothing else can verify either Issue** — ✅ **[I-0182]'s branch needs `listWorlds` to FAIL and [I-0195]'s cost needs a SLOW mount; Docker can stage neither.** |
