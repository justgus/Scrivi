...
| **T-0538** | ⚠️ **`[Cross]` Project open cost** — ⚠️ **kill the read amplification in `scrivi_open_project` ([I-0231]: 55,574 reads / 8,044 opens for 150 files; each sidecar ~96×) and get Linux's LANDING open off the UI thread ([I-0232]: 155 s in state `D` before [I-0195]'s async path is even entered).** ⚠️ **Measured on the real rig under `cache=none`: local `0.21 s` vs CIFS `154 s`.** ⛔ **The fix SHAPE is not chosen — AC1 is to LOCATE the amplification by measurement, not by reading.** | ⚠️ **EP-042** | 🟡 **ACTIVE — [SP-144]; see [`../Tasks/Task-active.md`](Task-active.md)** |
