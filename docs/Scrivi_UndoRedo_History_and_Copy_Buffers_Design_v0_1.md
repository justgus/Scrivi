...
- **Root floor (undo stop, clarified 2026-07-07):** the root holds each touched scene's **floor snapshot** — the state undo can restore *up to* but **never below**. Two origins, one rule:
  - *Project typed from scratch:* the floor is the empty scene. Undo walking back to empty is correct — that empty state genuinely is the project's history root.
  - *Project with pre-existing content when history is first created:* the floor is that **pre-existing text**, captured once as the root's per-scene `rec:"floor"` snapshot (Appendix A.1.a, `reason:"seed"`). Undo restores up to the pre-existing paragraph and stops — it must never destroy text the writer did not produce as a recorded event.

  Reaching the floor is a **hard stop with a clear notice** (`stoppedAtBarrier{kind:"historyStart", note:"Can't undo past the start of the recorded history."}`) — *not* a per-session wall. Crossing a **session** boundary *within* recorded history warns and continues (`crossedSessionBoundary`, below); only the true root floor and structural barriers are walls.
  - **Seeding lifecycle:** the floor is seeded **once, at history creation** (the `history/` package is first written), never re-seeded per launch or per session. *SP-053 stand-in:* while the engine is in-memory (opens empty every launch), the Swift capture layer seeds each scene's floor on its first edit that session via `scrivi_history_seed_scene` — behaviourally identical while nothing persists. *SP-054 (persistence):* the seed becomes the persisted root `rec:"floor"` record written once; on subsequent opens the scene's on-disk text is the **HEAD** (reached by replaying events from the floor), not the floor. A head-hash mismatch at open (the scene was edited outside Scrivi) is the **only** thing that re-seeds a scene's cached text — via an `externalChange` barrier that never mutates the manuscript (§6.b).
