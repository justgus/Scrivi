...
void TimelinePanel::mousePressEvent(QMouseEvent* event)
{
    if (event->button() == Qt::LeftButton) {
        ...
        if (i >= 0) {
            emit sceneClicked(dots_.at(i).sceneID);
            event->accept();
            return;
        }
    }
