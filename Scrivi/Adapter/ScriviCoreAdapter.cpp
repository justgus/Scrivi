...
// EP-009 — Scene and Chapter creation

std::string ScriviAdapter::createScene(
    const char* projectRootPath,
    const char* appSupportRoot,
    const char* projectID,
    ...
    const char* displayName)
{
    ...
    req.appSupportRoot  = appSupportRoot  ? appSupportRoot  : "";
    req.projectID       = scrivi::ProjectID{projectID   ? projectID   : ""};
    req.chapterID       = scrivi::ChapterID{chapterID   ? chapterID   : ""};
    req.afterSceneID    = scrivi::SceneID  {afterSceneID ? afterSceneID : ""};
    req.author = {
        scrivi::IdentityID{identityID   ? identityID   : ""},
        scrivi::PersonaID {personaID    ? personaID    : ""},
        displayName ? displayName : ""
    ...
    auto result = impl_->core->createScene(req);
    if (!result.ok()) { return errorEnvelope(result.error()); }
...
std::string ScriviAdapter::createChapter(
    const char* projectRootPath,
    const char* appSupportRoot,
    const char* projectID,
    const char* identityID,
    ...
    const char* displayName)
{
    ...
    auto result = impl_->core->createChapter(req);
    if (!result.ok()) { return errorEnvelope(result.error()); }
